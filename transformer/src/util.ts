...
/**
 * Like walk, but does not descend into nested function scopes.
 * Use this when you only want to see nodes that belong to the current function body.
 */
export function walkShallow(ts: typeof import("typescript"), node: ts.Node, visitor: (n: ts.Node) => void): void {
    if (!node) return;
    visitor(node);
