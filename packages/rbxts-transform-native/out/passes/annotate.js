...
exports.generateDLuau = generateDLuau;
...
function collectSidecar(ts, program, sourceFile) {
    ...
    function visit(node) {
        if (ts.isFunctionDeclaration(node) && node.name) {
            ...
            if (params.some(p => p !== null) || ret !== null || exported) {
                sidecar.fns.set(node.name.text, { params, paramNames, ret, exported });
            }
        }
...
function generateDLuau(luauPath, sidecar) {
    const exported = [...sidecar.fns.entries()].filter(([, ann]) => ann.exported);
    if (exported.length === 0)
        return;
    const dluauPath = luauPath.replace(/\.luau$/, ".d.luau");
    const lines = ["-- Generated by rbxts-transform-native", ""];
    // Individual function type aliases
    for (const [name, ann] of exported) {
        const paramList = ann.paramNames
            .map((pName, i) => {
            const typ = ann.params[i];
            return typ ? `${pName}: ${typ}` : pName;
        })
            .join(", ");
        const ret = ann.ret ?? "()";
        lines.push(`export type ${name} = (${paramList}) -> ${ret}`);
    }
    // Module table type
    lines.push("", "export type Module = {");
    for (const [name] of exported) {
        lines.push(`\t${name}: ${name},`);
    }
    lines.push("}");
    lines.push("");
    fs.writeFileSync(dluauPath, lines.join("\n"), "utf8");
}
const writingFiles = new Set();
function applyAnnotations(luauPath, sidecar, injectTypes, dluau) {
    ...
    if (dluau) {
        try {
            generateDLuau(luauPath, sidecar);
        }
        catch (err) {
            console.warn(`[native] failed to generate .d.luau for ${luauPath}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    if (changed) {
